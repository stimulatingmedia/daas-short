...
- **Glass surface + Depth Field pose** (the style reference): every UI element is glass — 16px frost, 62% white or 62% navy, hairline edge, inset highlight, navy-tinted shadow — floating in the Expressive profile's static pose (1200px perspective) over soft Nebula Blue / Royal Dark Ube glows. Copy stays on the front layer; blur is a material, never a motion. Light glass takes a Platinum Ice hairline (white on white is not an edge); navy glass carries its read in fill, edge and sheen rather than in frost, because a backdrop filter under a 3D-posed ancestor samples inconsistently frame to frame.
- **Where the pose tilts on both axes and where it doesn't.** Cards and chips take the full 12° two-axis pose. The outcome grid — six stacked statements, the densest reading column in the piece — tilts on X only at the mid rate, so baselines stay level and no caption falls under the 24px phone floor. `project()` solves where a posed point lands on screen, so the *your potential* arrow ends on the peak rather than beside it.
- **One neon line.** The S4 chart is the piece's single dark-glass surface. Stimulating Green on white measured about 1.5:1; on navy glass the line reads about 9:1, and the spec's "single neon line on `gradient-sky`" is literally true. A blurred twin sits just under the stroke so it reads as a lit tube; only the dash draws on.
- **One at a time in *The solution*:** the six feature cards land on the 12-frame beat grid (`T.beat` apart, so every start is a whole frame), each a two-line label with an icon tile that pops at 40% of its card's entrance. The scene was cut from 12s to 8s, the cloud ceiling removed so the sky is open above the rocket, and the scrolling service marquee replaced by a static list that lands with the sixth card — a loop could only show half its content in the time available, and continuous horizontal motion on running text is not one of the reveals the system sanctions.
- **Platform safe zones** are checked on the rendered frames, not by eye: copy clears the right action column and the bottom caption stack, and the watermark sits below the status strip.
